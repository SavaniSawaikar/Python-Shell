...
class TestShell(unittest.TestCase):
    ...
    def tearDownClass(cls):
        p = subprocess.run(["docker", "image", "rm", cls.TEST_IMAGE],
    ...
    def setUp(self):
        p = subprocess.run(["docker", "volume", "create", self.TEST_VOLUME],
                           stdout=subprocess.DEVNULL)
        if p.returncode != 0:
    ...
    def tearDown(self):
        p = subprocess.run(["docker", "volume", "rm", self.TEST_VOLUME],
                           stdout=subprocess.DEVNULL)
        if p.returncode != 0:
    ...
    def test_input_redirection_infront(self):  # TO DO
        cmdline = "< dir1/file2.txt cat"  # tbd
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "CCC")

    def test_input_redirection_nospace(self):  # TO DO
        cmdline = "cat <dir1/file2.txt"
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "CCC")

    def test_output_redirection(self):  # TO DO
    ...
    def test_output_redirection_overwrite(self):  # TO DO
    ...
    def test_globbing(self):  # TODO fail
        cmdline = "echo *.txt"
        stdout = self.eval(cmdline)
        result = set(stdout.strip().split())
        self.assertEqual(result, {"test.txt"})

    def test_globbing_dir(self):  # TODO fail
    ...
    def test_pipe_chain_sort_uniq(self):  # TO DO
    ...
    def test_substitution_semicolon(self):  # TODO fail
        cmdline = "echo `echo foo; echo bar`"  # tbd
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "foo bar")
    ...
    def test_substitution_app(self):  # TODO fail
        cmdline = "`echo echo` foo"
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "foo")

    def test_singlequotes(self):  # TO DO
        cmdline = "echo 'a  b'"  # tbd spaces
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "a  b")

    def test_quote_keyword(self):  # TO DO
        cmdline = "echo ';'"
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, ";")
    ...
    def test_substitution_doublequotes(self):  # identify
        cmdline = 'echo "`echo foo`"'
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "foo")

    def test_nested_doublequotes(self):  # identify
        cmdline = 'echo "a `echo "b"`"'
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, "a b")

    def test_disabled_doublequotes(self):  # TODO fail
        cmdline = "echo '\"\"'"
        stdout = self.eval(cmdline)
        result = stdout.strip()
        self.assertEqual(result, '""')
